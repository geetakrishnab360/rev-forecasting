...
with main_pane:
    ...
    if len(st.session_state["cohort_df"]) > 0:
        ...
        st.markdown("""
                        **cohort 0**: Deals with changed effective probability. Unselect only if you don't want to optimize these deals.
                        """)
        st.button(
        ...
        st.write("Set the maximum number of iterations for optimization")
        wcol = st.columns(3)
        with wcol[0]:
    ...
    with st.container():
        ...
        for expriment_name in ["Current"] + list(
            ...
            if expriment_name not in st.session_state["forecast_results"]:
                if expriment_name == "Current":
                    ...
                    _temp_ = fetch_and_forecast_experiment(expriment_name)
        ...
        st.markdown("""
                1. **Existing Approach**: Forecasts use the maximum of deal probability and effective probability.
                2. **Default**: Forecasts use deal probability.
                3. **Actuals**: Revenue from pipeline deals only.
                4. **Mᵢ**: Represents the iᵗʰ month from the snapshot date. Below are the averages of all Mᵢ’s across snapshots.
                """)
        st.table(
