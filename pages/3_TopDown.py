...
if 'is_fetched' not in st.session_state:
    st.session_state['all_bu_dfs'] = fetch_all_bu_dfs()
    st.session_state['all_bu_dfs'] = st.session_state['all_bu_dfs'].replace({'bu': {'dsx': 'Non In-sourcing','bts': 'In-sourcing'}})
    st.session_state['is_fetched'] = True

if 'max_actuals_date_times' not in st.session_state:
    st.session_state['max_actuals_date_times'] = {}
if 'max_actuals_date' not in st.session_state:
    st.session_state['max_actuals_date'] = {}
for bu in st.session_state['all_bu_dfs']['bu'].unique():
    st.session_state['max_actuals_date_times'][bu] = \
        st.session_state['all_bu_dfs'][st.session_state['all_bu_dfs']['bu'] == bu]['ds'].max()
    st.session_state['max_actuals_date'][bu] = datet.datetime.strptime(st.session_state['max_actuals_date_times'][bu],
                                                                       "%Y-%m-%d %H:%M:%S").date() + relativedelta(
        months=+1)
...
with left_pane:
    with st.expander('Data Inputs', expanded=False):
        if 'edit_bu' not in st.session_state:
            ...
            st.session_state['edit_bu']['Date'] = pd.to_datetime(
                [st.session_state['max_actuals_date'][bu] for bu in st.session_state['edit_bu']['BU']])
            ...
            if st.button('Save Edited Data', disabled=st.session_state['is_update']):
                ...
                if not selected_data.empty:
                    ...
                    for i, row in selected_data.iterrows():
                        ...
                        row_c = row_c.replace({'bu': {'Non In-sourcing': 'dsx', 'In-sourcing': 'bts'}})
                        if not existing_entry.empty:  # update
                            # update_bu_in_db(row_c, 'karthik')
                            update_rev_data(row_c, 'test')
                            # st.session_state['all_bu_dfs'].loc[
                            #     (st.session_state['all_bu_dfs']['bu'] == row['bu']) &
                            #     (st.session_state['all_bu_dfs']['ds'] == row['ds']), 'y'] = row['y']
                        else:  # insert
                            # insert_bu_into_db(row_c, 'karthik')
                            insert_new_rev_data(row_c, 'test')
                            # st.session_state['all_bu_dfs'] = pd.concat(
                            #     [st.session_state['all_bu_dfs'], row_c]).reset_index(
                            #     drop=True)
                    st.session_state['all_bu_dfs'] = fetch_all_bu_dfs()
                    st.session_state['all_bu_dfs'] = st.session_state['all_bu_dfs'].replace(
                        {'bu': {'dsx': 'Non In-sourcing', 'bts': 'In-sourcing'}})
                    cols = st.columns([0.4, 1])
        ...
        for bu in st.session_state['all_bu_dfs']['bu'].unique():
            st.session_state['max_actuals_date_times'][bu] = \
                st.session_state['all_bu_dfs'][st.session_state['all_bu_dfs']['bu'] == bu]['ds'].max()
            st.session_state['max_actuals_date'][bu] = datet.datetime.strptime(
                st.session_state['max_actuals_date_times'][bu], "%Y-%m-%d %H:%M:%S").date() + relativedelta(months=+1)
        ...
        no_of_past_years = min(st.session_state['max_actuals_date'].values()) + relativedelta(
            months=-1) - datet.datetime.strptime(
        ...
        if st.session_state['bu_graph_selected'] == "Overall":
            ...
            for bu in all_bu_dfs['bu'].unique():
                ...
                bu_data = bu_data[
                    bu_data['ds'] >= str(pd.to_datetime(st.session_state['max_actuals_date_times'][bu]) + relativedelta(
                        years=-int(st.session_state['past_years'])))]
                fig.add_trace(
                    go.Scatter(
                        x=bu_data['ds'],
            ...
            bu_data = bu_data[
                bu_data['ds'] >= str(pd.to_datetime(
                    st.session_state['max_actuals_date_times'][st.session_state['bu_graph_selected']]) + relativedelta(
                    years=-int(st.session_state['past_years'])))]
            fig.add_trace(
                go.Scatter(
                    x=bu_data['ds'],
    ...
    with st.expander('Future Forecasts', expanded=True):
        if 'fetch_forecast_data' not in st.session_state:
            ...
            st.session_state['fetch_forecast_data'] = st.session_state['fetch_forecast_data'].replace(
                {'BUSINESS_UNIT': {'dsx': 'Non In-sourcing', 'bts': 'In-sourcing'}})
        ...
        # forecast_df = forecast_df.replace({'bu': {'dsx': 'Non In-sourcing', 'bts': 'In-sourcing'}})
        ...
        if st.session_state['bu_forecast_selected'] == "Overall":
            for bu in st.session_state['all_bu_dfs']['bu'].unique():
                ...
                bu_data_actual = bu_data_actual[
                    (bu_data_actual['ds'] >= str(
                        pd.to_datetime(st.session_state['max_actuals_date_times'][bu]) + relativedelta(
                            years=-int(st.session_state['past_years']))))
                    &
                    (bu_data_actual['ds'] <= str(pd.to_datetime(st.session_state['max_actuals_date_times'][bu])))]
                fig.add_trace(
                    go.Scatter(
                        x=bu_data_actual['ds'],
                        y=bu_data_actual['y'],
            ...
            bu_data_actual = bu_data_actual[
                (bu_data_actual['ds'] >= str(
                    pd.to_datetime(st.session_state['max_actuals_date_times'][
                                       st.session_state['bu_forecast_selected']]) + relativedelta(
                        years=-int(st.session_state['past_years']))))
                &
                (bu_data_actual['ds'] <= str(pd.to_datetime(st.session_state['max_actuals_date_times'][st.session_state[
                    'bu_forecast_selected']]) + relativedelta(months=+1)))]
            fig.add_trace(
                go.Scatter(
                    x=bu_data_actual['ds'],
                    y=bu_data_actual['y'],
            ...
            for model in st.session_state['selected_models']:
                ...
                fig.add_trace(
                    go.Scatter(
                        ...
                        mode='lines',
                        line=dict(dash='dot'),
                        name=f"{st.session_state['bu_forecast_selected']}_Forecast_{model}"
                    )
                )
        fig.update_layout(
        ...
        for bu in download_table['bu'].unique():
            temp_bu_df = download_table[download_table['bu'] == bu]
            temp_bu_df = temp_bu_df[(temp_bu_df['ds'] >= str(
                pd.to_datetime(st.session_state['max_actuals_date_times'][bu]) + relativedelta(
                    months=+1)))
                                    &
                                    (temp_bu_df['ds'] <= str(
                                        pd.to_datetime(st.session_state['max_actuals_date_times'][bu]) + relativedelta(
                                            months=+st.session_state['forecast_months'])))]
            download_table[download_table['bu'] == bu] = temp_bu_df
        ...
        forecast_table['Business Unit'] = forecast_table['Business Unit']
        for i, model in enumerate(st.session_state['selected_models']):
